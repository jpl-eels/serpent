function statistics_data = extract_statistics(data)
    ...
    for i = 1:statistics_data.num_stats
        ...
        entry.match_ids = cell(length(statistics), 1);
        for j = 1:length(statistics)
            entry.match_ids{j} = statistics{j}.MatchIds;
        end
        [entry.frames_tracked, entry.frames_tracked_bins, ...
            entry.mean_frames_tracked, entry.min_frames_tracked, ...
            entry.max_frames_tracked, entry.max_track_duration] = ...
            compute_frame_track_statistics(entry.match_ids);
