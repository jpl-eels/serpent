...
class PointcloudNormalEstimation {
    ...
    static_assert(
            std::is_floating_point<decltype(PointOut::curvature)>::value, "curvature is not a floating point type");
...
void compute_in_place(
        const typename pcl::PointCloud<PointT>::Ptr pointcloud, typename pcl::Feature<PointT, PointT>& feature) {
...
#include "serpent/front_end/lidar/pointcloud_normal_estimation-inl.hpp"
