...
#include "serpent/front_end/lidar/registration.hpp"
        ...
        if (covariance_estimation_method == CovarianceEstimationMethod::CENSI ||
            ...
            switch (point_covariance_method) {
                ...
                case PointCovarianceMethod::RANGE_BIAS:
                    covariance = range_bias_covariance(
                            registration, range_variance, range_bias_variance, correspondence_count);
                    break;
                default:
...
template<typename PointT>
Registration<PointT>::Registration() : nh("serpent"), s2s_sync(10), body_frames("serpent") {
    // Publishers
...
template<typename PointT>
void Registration<PointT>::publish_refined_transform(
        const Eigen::Matrix4d transform, const Eigen::Matrix<double, 6, 6> covariance, const ros::Time& timestamp) {
...
template<typename PointT>
void Registration<PointT>::s2s_callback(
        const PointCloudConstPtr& current_pointcloud, const geometry_msgs::TransformStamped::ConstPtr& transform_msg) {
...
void Registration<PointT>::s2m_callback(const PointCloudConstPtr& map) {
    ...
    // Publish the refined transform
    publish_refined_transform(
            s2m_transform, covariance_from_registration(*s2m), pcl_conversions::fromPCL(pointcloud->header.stamp));
}
