...
private:
    using PointVarianceCovarianceFunction = typename Eigen::Matrix<double, 6, 6> (*)(
            PCLRegistration& registration, const double point_variance, int& correspondence_count);
    using RangeCovarianceFunction = typename Eigen::Matrix<double, 6, 6> (*)(
            PCLRegistration& registration, const double range_variance, int& correspondence_count);
     ...
     * @param timestamp
     */
    void publish_refined_transform(
     ...
     */
    void s2s_callback(
            const PointCloudConstPtr& pointcloud_msg, const geometry_msgs::TransformStamped::ConstPtr& transform_msg);
...
#include "serpent/front_end/lidar/registration-inl.hpp"
