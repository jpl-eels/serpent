...
#include "serpent/front_end/stereo/stereo_equations.hpp"
...
template<typename Scalar>
Eigen::Matrix<Scalar, 3, 1> stereo_project(
        const Eigen::Matrix<Scalar, 3, 1>& point, const Eigen::Matrix<Scalar, 3, 3>& intrinsic, const Scalar baseline) {
    if (point[3] <= 0.0) {
    ...
    return Eigen::Matrix<Scalar, 3, 1>{intrinsic(0, 2), intrinsic(0, 2), intrinsic(1, 2)} +
           Eigen::Matrix<Scalar, 3, 1>{
                   intrinsic(0, 0) * point[0], intrinsic(0, 0) * (point[0] - baseline), intrinsic(1, 1) * point[1]} /
                   point[3];
}
